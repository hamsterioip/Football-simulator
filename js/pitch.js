...
(function (global) {
  ...
  const Pitch = {
    ...
    celebrate(root, opts, done) {
      ...
      const flag = q('.pg-flag');
      // the corner he makes for, and the flag waiting there
      const corner = { x: toLeft ? 58 : W - 58, y: 190 };
      const flagAt = { x: toLeft ? 32 : W - 32, y: 192 };
      const camFocus = corner.y - 26;

      if (reduced()) {
        sc.style.opacity = '1';
        sc.style.transform = place(corner.x, corner.y, 0, 1.05);
        if (flag) { flag.style.opacity = '1'; flag.style.transform = place(flagAt.x, flagAt.y); }
        const camr = q('.pg-cam'); if (camr) camr.style.transform = about(corner.x, camFocus, 0, 1.22);
        if (done) done();
        return;
      }
      ...
      /* ---- the corner flag, planted and fluttering where he is heading ---- */
      if (flag) {
        flag.style.opacity = '1';
        flag.style.transform = place(flagAt.x, flagAt.y);
      ...
      const flagWobble = delay => {
        if (flag) flag.animate([
          { transform: place(flagAt.x, flagAt.y, 0) },
          { transform: place(flagAt.x, flagAt.y, dir * 11), offset: .28 },
          { transform: place(flagAt.x, flagAt.y, -dir * 7), offset: .55 },
          { transform: place(flagAt.x, flagAt.y, dir * 3), offset: .78 },
          { transform: place(flagAt.x, flagAt.y, 0) }
        ], { duration: 720, delay, easing: 'ease-out', fill: 'forwards' });
      };

      /* ---- phase one: the sprint to the corner, camera swings with him ---- */
      const RUN = 720;
      const midX = SPOT.x + (corner.x - SPOT.x) * .55;
      sc.animate([
        { transform: place(SPOT.x - dir * 18, H + 44, 0, .8) },
        { transform: place(midX, 204, dir * 4, .93), offset: .45 },
        { transform: place(corner.x - dir * 30, corner.y + 7, dir * 2, 1.02), offset: .8 },
        { transform: place(corner.x, corner.y, 0, 1.06) }
      ], { duration: RUN, easing: ease, fill: 'forwards' });
      if (body) body.animate([                        // leaning into the sprint
        { transform: 'rotate(0deg)' },
        { transform: `rotate(${dir * 10}deg)`, offset: .35 },
        { transform: `rotate(${dir * 7}deg)`, offset: .8 },
        { transform: 'rotate(0deg)' }
      ], { duration: RUN, easing: ease, fill: 'forwards' });
      arms([[0, 0, 0], [-32, 32, .3], [26, -26, .62], [-14, 14, 1]],
        { duration: RUN, easing: 'ease-in-out', fill: 'forwards' });   // pumping
      if (shadow) shadow.animate([
        { transform: 'scale(.85,1)', opacity: .36 },
      ...
      if (cam) cam.animate([
        { transform: about(160, 150, 0, 1.01) },
        { transform: about(corner.x, camFocus, 0, 1.2) }
      ], { duration: RUN + 140, easing: 'cubic-bezier(.3,.6,.3,1)', fill: 'forwards' });

      /* ---- phase two: the routine, at the flag ---- */
      let dur = RUN + 800, camTo = 1.26;
      const rOpt = extra => Object.assign({ delay: RUN, easing: 'ease-out', fill: 'forwards' }, extra || {});

      if (style === 'slide') {
        // drops to his knees and skids past the flag
        sc.animate([
          { transform: place(corner.x, corner.y, 0, 1.06) },
          { transform: place(corner.x + dir * 14, corner.y + 3, 0, 1.06), offset: .55 },
          { transform: place(corner.x + dir * 20, corner.y + 4, 0, 1.05) }
        ], rOpt({ duration: 620, easing: ease }));
        if (body) body.animate([
          { transform: 'rotate(0deg) scaleY(1)' },
          { transform: `rotate(${dir * 8}deg) scaleY(1.02)`, offset: .3 },
          { transform: `rotate(${dir * 20}deg) scaleY(.88)`, offset: .72 },
          { transform: `rotate(${dir * 16}deg) scaleY(.91)` }
        ], rOpt({ duration: 620, easing: ease }));
        arms([[-14, 14, 0], [-10, 10, .3], [36, -36, 1]], rOpt({ duration: 620 }));
        if (shadow) shadow.animate([{ transform: 'scale(1.1,1)', opacity: .34 },
          { transform: 'scale(1.6,.75)', opacity: .26 }], rOpt({ duration: 620 }));
        flagWobble(RUN + 240);
        dur = RUN + 760; camTo = 1.28;

      } else if (style === 'leap') {
        sc.animate([
          { transform: place(corner.x, corner.y, 0, 1.06) },
          { transform: place(corner.x, corner.y - 38, 360, 1.12), offset: .5 },   // up and round
          { transform: place(corner.x, corner.y + 2, 360, 1.06), offset: .82 },
          { transform: place(corner.x, corner.y, 360, 1.06) }
        ], rOpt({ duration: 820, easing: 'cubic-bezier(.3,.65,.3,1)' }));
        arms([[-14, 14, 0], [-44, 44, .35], [26, -26, .7], [8, -8, 1]], rOpt({ duration: 820 }));
        if (shadow) shadow.animate([
          { transform: 'scale(1.05)', opacity: .36 },
          { transform: 'scale(.5)', opacity: .12, offset: .5 },
          { transform: 'scale(1.15)', opacity: .36 }
        ], rOpt({ duration: 820 }));
        flagWobble(RUN + 500);
        dur = RUN + 950; camTo = 1.3;

      } else if (style === 'wings') {
        const loop = dir * 30;                           // banks round the flag
        sc.animate([
          { transform: place(corner.x, corner.y, 0, 1.06) },
          { transform: place(corner.x + loop * .5, corner.y - 8, 0, 1.08), offset: .4 },
          { transform: place(corner.x + loop, corner.y + 1, 0, 1.06), offset: .8 },
          { transform: place(corner.x + loop * .9, corner.y + 2, 0, 1.06) }
        ], rOpt({ duration: 900, easing: 'linear' }));
        if (body) body.animate([
          { transform: 'rotate(0deg)' },
          { transform: `rotate(${-dir * 13}deg)`, offset: .45 },
          { transform: `rotate(${-dir * 9}deg)` }
        ], rOpt({ duration: 900, easing: 'ease-in-out' }));
        arms([[-14, 14, 0], [-72, 72, .25], [-78, 78, 1]], rOpt({ duration: 900 }));
        flagWobble(RUN + 420);
        dur = RUN + 1020;
        ...
        sc.animate([
          { transform: place(corner.x, corner.y, 0, 1.06) },
          { transform: place(corner.x, corner.y + 8, 0, 1.16), offset: .6 },   // right at the lens
          { transform: place(corner.x, corner.y + 10, 0, 1.2) }
        ], rOpt({ duration: 900 }));
        arms([[-14, 14, 0], [-30, 4, .35], [-96, 6, .62], [-96, 6, 1]], rOpt({ duration: 900 }));
        dur = RUN + 1050; camTo = 1.34;

      } else if (style === 'heart') {
        sc.animate([
          { transform: place(corner.x, corner.y, 0, 1.06) },
          { transform: place(corner.x, corner.y - 6, 0, 1.07), offset: .5 },   // the little hop
          { transform: place(corner.x, corner.y, 0, 1.07) }
        ], rOpt({ duration: 700 }));
        arms([[-14, 14, 0], [-40, 40, .35], [-104, 104, .68], [-100, 100, 1]],
          rOpt({ duration: 900, easing: 'cubic-bezier(.3,1.4,.5,1)' }));
        dur = RUN + 1050; camTo = 1.3;

      } else if (style === 'badge') {
        sc.animate([
          { transform: place(corner.x, corner.y, 0, 1.06) },
          { transform: place(corner.x, corner.y + 1, 0, 1.07) }
        ], rOpt({ duration: 500 }));
        ...
        dur = RUN + 1100; camTo = 1.36;                       // slow push while he does nothing

      } else if (style === 'mobbed') {
        sc.animate([
          { transform: place(corner.x, corner.y, 0, 1.06) },
          { transform: place(corner.x, corner.y + 2, 0, 1.04), offset: .7 },
          { transform: place(corner.x, corner.y + 1, 0, 1.05) }
        ], rOpt({ duration: 900 }));
        arms([[-14, 14, 0], [-46, 46, .4], [-34, 34, 1]], rOpt({ duration: 900 }));
        // team-mates pile in from both sides of the frame
        root.querySelectorAll('.pg-mate').forEach((mate, i) => {
          const from = i % 2 ? 1 : -1;
          mate.style.opacity = '1';
          mate.animate([
            { transform: place(corner.x + from * 190, 202, 0, .74), opacity: 0 },
            { transform: place(corner.x + from * 190, 202, 0, .78), opacity: 1, offset: .12 },
            { transform: place(corner.x + from * (26 + i * 9), corner.y + 1, 0, .94), offset: .72 },
            { transform: place(corner.x + from * (22 + i * 9), corner.y + 3, 0, .96) }
          ], { duration: 1000, delay: RUN + 160 + i * 120, easing: 'cubic-bezier(.3,.7,.35,1)', fill: 'forwards' });
        });
        flagWobble(RUN + 700);
        dur = RUN + 1250; camTo = 1.24;
      }

      // camera: settles on him at the flag, then a slow punch in
      if (cam) cam.animate([
        { transform: about(corner.x, camFocus, 0, 1.2) },
        { transform: about(corner.x, camFocus, 0, camTo), offset: .45 },
        { transform: about(corner.x, camFocus, 0, camTo - .02) }
      ], { duration: dur - RUN + 200, delay: RUN, easing: 'ease-out', fill: 'forwards' });
